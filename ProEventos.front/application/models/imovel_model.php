<?php

class Imovel_model extends CI_Model {
    /*
      | -------------------------------------------------------------------
      | configuração do construtor
      | -------------------------------------------------------------------
      | $this->tab              o nome da tabela principal
      |
      | -------------------------------------------------------------------
     */

    public function __construct() {
        parent::__construct();
        $this->tab = "imovel";
    }

    // --------------------------------------------------------------------

    /*
      | -------------------------------------------------------------------
      | pega todos os registros da tabela
      |
      | -------------------------------------------------------------------
     */

    public function get() {
        $this->db->select(array('i.*', 'f.nome as fases', 'f.cor as fases_cor'));
        $this->db->from($this->tab . ' as i');
        $this->db->join('fasesempreendimento as f', 'i.status = f.id', 'left');
        $this->db->order_by("i.id", "desc");
        return $this->db->get()->result_array();
    }

    /*
      | -------------------------------------------------------------------
      | pega todos os registros da tabela
      |
      | -------------------------------------------------------------------
     */

    public function getImoveis() {
        $this->db->select(array('i.*', 'f.nome as fases', 'f.cor as fases_cor', 'f.background as fases_background'));
        $this->db->from($this->tab . ' as i');
        $this->db->join('fasesempreendimento as f', 'i.status = f.id', 'left');
        $this->db->order_by("i.order", "asc");
        $this->db->order_by("i.id", "desc");
        return $this->db->get()->result_array();
    }

    /*
      | -------------------------------------------------------------------
      | pega todos os registros da tabela
      |
      | -------------------------------------------------------------------
     */

    public function getLimit() {
        $this->db->select(array('i.*', 'f.nome as fases', 'f.cor as fases_cor'));
        $this->db->from($this->tab . ' as i');
        $this->db->join('fasesempreendimento as f', 'i.status = f.id', 'left');
        $this->db->order_by("i.id", "desc");
        $this->db->limit("4");
        return $this->db->get()->result_array();
    }

    /*
      | -------------------------------------------------------------------
      | pega os registros da tabela com a id igual a que foi enviada
      | por parametro
      |
      | -------------------------------------------------------------------
     */

    public function getByID($id) {
        $this->db->select(array('i.*', 'f.nome as fases', 'f.cor as fases_cor'));
        $this->db->from($this->tab . ' as i');
        $this->db->join('fasesempreendimento as f', 'i.status = f.id', 'left');
        $this->db->where("i.id", $id);
        return $this->db->get()->row_array();
    }

    /*
      | -------------------------------------------------------------------
      | pega os registros da tabela com a id igual a que foi enviada
      | por parametro
      |
      | -------------------------------------------------------------------
     */

    public function getByURL($url) {
        $this->db->select(array('i.*', 'f.nome as fases', 'f.cor as fases_cor'));
        $this->db->from($this->tab . ' as i');
        $this->db->join('fasesempreendimento as f', 'i.status = f.id', 'left');
        $this->db->where("i.url", $url);
        return $this->db->get()->row_array();
    }

    /*
      | -------------------------------------------------------------------
      | pega os registros da tabela com a id igual a que foi enviada
      | por parametro
      |
      | -------------------------------------------------------------------
     */

    public function getByFase($fase, $tipo) {
        $this->db->select(array('i.*', 'f.nome as fases', 'f.cor as fases_cor'));
        $this->db->from($this->tab . ' as i');
        $this->db->join('fasesempreendimento as f', 'i.status = f.id', 'left');
        $this->db->where("i.status", $fase);
        $this->db->order_by("i.id", "desc");
        return $this->db->get()->result_array();
    }

    /*
      | -------------------------------------------------------------------
      | pega os registros da tabela com a id igual a que foi enviada
      | por parametro
      |
      | -------------------------------------------------------------------
     */

    public function getResidencial() {
        $this->db->select(array('*'));
        $this->db->from($this->tab);
        $this->db->order_by("id", "desc");
        return $this->db->get()->result_array();
    }

    /*
      | -------------------------------------------------------------------
      | pega os registros da tabela com a id igual a que foi enviada
      | por parametro
      |
      | -------------------------------------------------------------------
     */

    public function getComercial() {
        $this->db->select(array('*'));
        $this->db->from($this->tab);
        $this->db->order_by("id", "desc");
        return $this->db->get()->result_array();
    }

    /*
      | -------------------------------------------------------------------
      | pega os registros da tabela com a id igual a que foi enviada
      | por parametro
      |
      | -------------------------------------------------------------------
     */

    public function getDestaque($tipo) {
        $this->db->select(array('*'));
        $this->db->from($this->tab);
        $this->db->where("destaque", "1");
        return $this->db->get()->result_array();
    }

    /*
      | -------------------------------------------------------------------
      | pega os registros da tabela com a id igual a que foi enviada
      | por parametro
      |
      | -------------------------------------------------------------------
     */

    public function getHome() {
        $this->db->select(array('i.*', 'f.nome as fases', 'f.cor as fases_cor'));
        $this->db->from($this->tab . ' as i');
        $this->db->join('fasesempreendimento as f', 'i.status = f.id', 'left');
        $this->db->where("i.home", "1");
        return $this->db->get()->result_array();
    }

    /*
      | -------------------------------------------------------------------
      | pega os registros da tabela com a id igual a que foi enviada
      | por parametro
      |
      | -------------------------------------------------------------------
     */

    public function getQuery($quero, $tipo, $where) {
        $this->db->select(array('*'));
        $this->db->from($this->tab);
        if ($quero) {
            $this->db->like("objetivo_imovel", $quero);
        }
        if ($tipo) {
            $this->db->like("tipo", $tipo);
        }
        $this->db->where($where);
        $this->db->where("objetivo_imovel <>", "50");
        $this->db->order_by("status", "asc");
        $this->db->order_by("tipo", "asc");
        $this->db->order_by("id", "desc");
        return $this->db->get()->result_array();
    }

    // --------------------------------------------------------------------

    /*
      | -------------------------------------------------------------------
      | inclui os dados no banco
      |
      | -------------------------------------------------------------------
     */

    public function set($info) {
        $add = $this->db->insert($this->tab, $info);
        if ($add) {
            return TRUE;
        } else {
            return FALSE;
        }
    }

    // --------------------------------------------------------------------

    /*
      | -------------------------------------------------------------------
      | atualiza os dados no banco
      |
      | -------------------------------------------------------------------
     */

    public function up($info) {
        $this->db->where("id", $info['id']);
        $up = $this->db->update($this->tab, ($info));
        if ($up) {
            return TRUE;
        } else {
            return FALSE;
        }
    }

    // --------------------------------------------------------------------

    /*
      | -------------------------------------------------------------------
      | apaga o registro no banco
      |
      | -------------------------------------------------------------------
     */

    public function del($id) {
        $this->db->where("id", $id);
        $del = $this->db->delete($this->tab);
        if ($del) {
            return TRUE;
        } else {
            return FALSE;
        }
    }

}
